 ...
 * Catatan: Seedream edit model hanya support 1 image_url (singular),
 * jadi style reference digabung dalam prompt untuk memandu AI.
...
export async function generateWithStyleReference(
  ...
  try {
    // Upload foto wajah ke storage (Seedream hanya support 1 image_url)
    const imageUrl = await uploadToFalStorage(imageBase64);
