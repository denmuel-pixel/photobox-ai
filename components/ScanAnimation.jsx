...
export default function ScanAnimation({ isVisible, onComplete, imageSrc }) {
  ...
  return (
    ...
    >
      ...
      {/* Scan line bergerak di atas foto */}
      <motion.div
        ref={scanLineRef}
      ...
      <div className="absolute top-2 left-2 z-20 flex gap-1">
        <span className="w-3 h-3 border-t-2 border-l-2 border-primary/60 rounded-tl" />
      </div>
      <div className="absolute top-2 right-2 z-20 flex gap-1">
        <span className="w-3 h-3 border-t-2 border-r-2 border-secondary/60 rounded-tr" />
      </div>
      <div className="absolute bottom-2 left-2 z-20 flex gap-1">
        <span className="w-3 h-3 border-b-2 border-l-2 border-primary/60 rounded-bl" />
      </div>
      <div className="absolute bottom-2 right-2 z-20 flex gap-1">
        <span className="w-3 h-3 border-b-2 border-r-2 border-secondary/60 rounded-br" />
      </div>

      {/* Status text - bottom center */}
      <AnimatePresence mode="wait">
        <motion.div
          key={currentStep >= 0 ? scanSteps[currentStep]?.id || "done" : "waiting"}
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          exit={{ opacity: 0, y: -10 }}
          className="absolute bottom-3 left-1/2 -translate-x-1/2 z-20"
        >
          <span className="inline-flex items-center gap-2 bg-black/60 backdrop-blur-sm text-white text-xs font-medium px-3 py-1.5 rounded-full">
        ...
        {scanSteps.map((step, i) => (
          <motion.div
            ...
            className={`rounded-full transition-all ${
              i <= currentStep ? "bg-primary" : "bg-white/30"
            }`}
            animate={{
